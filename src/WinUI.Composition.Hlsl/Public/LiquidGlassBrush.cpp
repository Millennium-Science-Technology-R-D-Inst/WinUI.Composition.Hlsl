...
namespace winrt::WinUI::Composition::Hlsl::implementation
{
	...
	{
		ValidateRange(value, 0.0, 64.0, L"BlurRadius must be between 0 and 64 DIPs.");
	...
	{
		ValidateRange(value, 0.0, 128.0, L"RefractionStrength must be between 0 and 128.");
	...
	{
		ValidateRange(value, 0.0, 16.0, L"DispersionStrength must be between 0 and 16.");
	...
	{
		ValidateRange(value, 0.0, 512.0, L"CornerRadius must be between 0 and 512 DIPs.");
	...
	{
		ValidateRange(value, 0.0, 32.0, L"BorderThickness must be between 0 and 32 DIPs.");
	...
	{
		ValidateRange(value, 0.0, 4.0, L"HighlightStrength must be between 0 and 4.");
	...
	void LiquidGlassBrush::BezelWidth(double value)
	{
		ValidateRange(value, 1.0, 256.0, L"BezelWidth must be between 1 and 256 DIPs.");
		SetValue(BezelWidthProperty(), box_value(value));
	}
	...
	void LiquidGlassBrush::GlassThickness(double value)
	{
		ValidateRange(value, 0.0, 256.0, L"GlassThickness must be between 0 and 256 DIPs.");
		SetValue(GlassThicknessProperty(), box_value(value));
	}
	...
	void LiquidGlassBrush::RefractiveIndex(double value)
	{
		ValidateRange(value, 1.0, 3.5, L"RefractiveIndex must be between 1 and 3.5.");
		SetValue(RefractiveIndexProperty(), box_value(value));
	}
	...
	void LiquidGlassBrush::TintOpacity(double value)
	{
		ValidateRange(value, 0.0, 1.0, L"TintOpacity must be between 0 and 1.");
		SetValue(TintOpacityProperty(), box_value(value));
	}
	...
	void LiquidGlassBrush::Saturation(double value)
	{
		ValidateRange(value, 0.0, 4.0, L"Saturation must be between 0 and 4.");
		SetValue(SaturationProperty(), box_value(value));
	}
	...
	void LiquidGlassBrush::LightAngle(double value)
	{
		ValidateRange(value, -6.2831855, 6.2831855, L"LightAngle must be between -2pi and 2pi radians.");
		SetValue(LightAngleProperty(), box_value(value));
	}
