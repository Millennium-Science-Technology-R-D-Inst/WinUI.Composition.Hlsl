...
- `Color`、`Sampler`、`MaterializedSampler` 三种公开 Composition shader contract。
...
- 包内生成的 shader library 自带 build-time `Kind` / `Profile` 元数据，不需要调用方重复声明。
...
编译器还会在同一个 shader library 内写入保留的 metadata export，用于记录本次 MSBuild 选择的 `HlslEffectKind` 和 `HlslShaderProfile`。`HlslShaderLibrary` 接受 generated bytecode 时，会同时反射真实 `PSBody` ABI 并校验 metadata，而不是盲信标记。
