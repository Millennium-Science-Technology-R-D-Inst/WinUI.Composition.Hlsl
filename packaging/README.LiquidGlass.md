...
WinUI `AutoSuggestBox` is sealed, so `LiquidGlassSearchBox` is a composable wrapper around a real native `AutoSuggestBox`. The wrapper mirrors the search-specific dependency-property surface exported by the Windows App SDK contract consumed by the package (`Text`, `PlaceholderText`, suggestion-list state and sizing, `TextMemberPath`, `Header`, `AutoMaximizeSuggestionArea`, `UpdateTextOnSelect`, `TextBoxStyle`, `QueryIcon`, `LightDismissOverlayMode`, `Description`) plus the commonly used inherited `ItemsSource` and `ItemTemplate` properties. These wrapper properties expose public `<Name>Property` identifiers, support normal XAML binding including TwoWay `x:Bind`, and synchronize changes in both directions with the hosted control. It also forwards `SuggestionChosen`, `TextChanged`, and `QuerySubmitted` with the original WinUI sender/event-args signatures. `InnerAutoSuggestBox` remains available for advanced or future-version APIs that are intentionally not mirrored.
