...
A future build-transitive NuGet target can compile consumer `.hlsl` files with the same `FxCompile` settings and generate descriptors automatically. That is deliberately separate from this change: the first step is to make the runtime payload model and built-in LiquidGlass use precompiled bytecode correctly.
