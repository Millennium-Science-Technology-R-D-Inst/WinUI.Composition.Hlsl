...
- `Color`, `Sampler`, and public `MaterializedSampler` Composition shader contracts.
...
- Self-describing package-generated shader libraries: build-time `Kind` and `Profile` travel with the compiled bytecode.
...
The compiler also emits a reserved metadata export containing the selected `HlslEffectKind` and `HlslShaderProfile`. `HlslShaderLibrary` validates that metadata against the actual reflected `PSBody` ABI before accepting generated bytecode.
...
later:     cache -> LoadGeneratedFromFileAsync/CreateFromGeneratedByteArray
                 -> HlslEffect.CreateCompiled -> Composition
```
